...
action :install do
  unless @zenpack.exists
    ...
    remote_file "/tmp/#{zpfile}" do
      source "#{new_resource.base_url}/#{zpfile}"
      mode "0644"
      action :create
    end
    #install the ZenPack
  ...
  if @zenpack.exists
    ...
    execute "zenpack --remove" do
      ...
      environment ({
                     ...
                     'PYTHONPATH' => "#{node[:zenoss][:server][:zenhome]}/lib/python",
                     'ZENHOME' => node[:zenoss][:server][:zenhome],
                     'PATH' => "#{node[:zenoss][:server][:zenhome]}/bin:${PATH}",
                     'PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION' => 'cpp'
                   })
...
def load_current_resource
  ...
  if @@zp_output.empty?
       Chef::Log.debug("Querying zenpacks from zenoss server.")
       zp = shell_out("sudo -u zenoss -i #{node[:zenoss][:server][:zenhome]}/bin/zenpack --list")
       @@zp_output = zp.stdout
  end
  exists = @@zp_output.include?(new_resource.package)
